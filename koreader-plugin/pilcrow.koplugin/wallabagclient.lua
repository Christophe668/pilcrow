...
--- Create a Wallabag annotation on `entry_id`. Wallabag has a richer
--  annotation contract than Readeck: `quote` is a first-class field,
--  so the highlighted text round-trips natively (no need to fold it
--  into the user note). `ranges` is an array of XPath / offset pairs;
--  the format matches Wallabag's web client's range serializer.
--    payload = {
--      quote  = "...",       -- required
--      ranges = { {          -- required, at least one
--          start       = "...",  -- XPath
--          startOffset = 0,
--          ["end"]     = "...",  -- XPath
--          endOffset   = 0,
...
--    }
function Client:createAnnotation(entry_id, payload)
    local body = {
        quote  = payload.quote or "",
        ranges = payload.ranges or {},
    }
    if payload.text and payload.text ~= "" then body.text = payload.text end
    return with_auth(self, function()
        return self:_request("POST",
            string.format("/api/annotations/%s.json", tostring(entry_id)),
            JSON.encode(body),
            { ["Content-Type"] = "application/json" })
    end)
end

--- List Wallabag annotations on `entry_id`. Returns `(true, list)` where
--  list is an array of `{ id, quote, text, ranges, created_at, ... }`.
--  Wallabag wraps the result as `{ total, rows }`; we unwrap it so
--  callers see a flat array regardless of backend.
function Client:listAnnotations(entry_id)
