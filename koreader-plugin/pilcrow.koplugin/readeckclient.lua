...
--- Create a Readeck annotation under `bookmark_id`. `payload` must
--  carry the four selector fields; `note` is optional.
--    payload = {
--      start_selector = "...",  -- XPath string
--      start_offset   = 0,
--      end_selector   = "...",
--      end_offset     = 0,
--      note           = "...",  -- optional
--    }
--  Returns `(true, decoded_annotation)` on success.
function Client:createAnnotation(bookmark_id, payload)
    local body = {
        start_selector = payload.start_selector or "",
        start_offset   = payload.start_offset or 0,
        end_selector   = payload.end_selector or payload.start_selector or "",
        end_offset     = payload.end_offset or 0,
    }
    if payload.note and payload.note ~= "" then body.note = payload.note end
    if payload.color and payload.color ~= "" then body.color = payload.color end
    return with_auth(self, function()
        return self:_request("POST",
            bookmark_path(bookmark_id) .. "/annotations",
            JSON.encode(body))
    end)
end

--- List annotations under `bookmark_id`. Returns `(true, list)` where
--  list is an array of `{ id, text, note, created, start_selector, ... }`.
function Client:listAnnotations(bookmark_id)
    return with_auth(self, function()
        return self:_request("GET",
            bookmark_path(bookmark_id) .. "/annotations")
    end)
