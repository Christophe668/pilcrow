...
const ANNOTATIONS: readonly FilterRow[] = [
  { route: "/(app)/(library)/highlights" as Href, label: "Highlights", segment: "highlights" },
  { route: "/(app)/(library)/notes" as Href, label: "Notes", segment: "notes" },
];
...
export function Rail() {
  ...
  const inStats = segments.includes("stats");

  return (
    <View className="flex-1 bg-bg">
      <ScrollView className="flex-1" contentContainerClassName="px-5 pt-8 pb-6">
              ...
              <Link key={f.label} href={f.route} asChild>
                ...
                >
                  <View className="flex-row items-center justify-between">
                    ...
                    {typeof n === "number" ? (
                      <Text className={countClass(isActive)}>{n}</Text>
                    ) : null}
                  </View>
                </Pressable>
              </Link>
        ...
        <View className="mb-8">
          {ANNOTATIONS.map((f) => {
            const isActive = segments.includes(f.segment);
            return (
              <Link key={f.label} href={f.route} asChild>
                <Pressable
                  className={`${rowClass(isActive)} hover:bg-surface-2 active:bg-surface-2`}
      ...
      {/* Pinned footer — stays anchored at the bottom of the rail
          regardless of how long the tags list scrolls. The 1px top
          rule visually separates it from the scrolling region above. */}
      <View className="border-t border-border px-5 py-3">
        <Link href={"/(app)/(library)/stats" as Href} asChild>
          <Pressable className={`${rowClass(inStats)} hover:bg-surface-2 active:bg-surface-2`}>
            <View className="flex-row items-center">
              <Feather
                name="bar-chart-2"
                size={14}
                color={inStats ? tokens.accent : tokens.muted}
              />
              <Text className={`${labelClass(inStats)} ml-2`}>Stats</Text>
            </View>
          </Pressable>
        </Link>
