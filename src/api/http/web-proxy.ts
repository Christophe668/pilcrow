...
export function proxiedFetch(url: string, init?: RequestInit): Promise<Response> {
  ...
  return fetch(rewritten, { ...init, headers }).catch((e) => {
    // Browsers throw a bare `TypeError: Failed to fetch` for every
    // network-layer failure — useless without context. Re-wrap so the
    // dev console shows which URL we were trying to hit and what the
    // upstream target was.
    const msg = e instanceof Error ? e.message : String(e);
    const wrapped = new Error(
      `[proxy] ${init?.method ?? "GET"} ${rewritten} → ${target}${parsed.pathname}${parsed.search} failed: ${msg}`,
    );
    wrapped.cause = e;
    throw wrapped;
  });
