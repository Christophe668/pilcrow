...
export async function upsertArticleByBackendId(db: DbDriver, row: ArticleUpsert): Promise<number> {
  return db.transaction(async (tx) => {
    ...
    // through to autoincrement. Pre-checking (vs. try/catch on the
    // INSERT) keeps the surrounding transaction alive on web SQLite,
    // where a constraint-aborting statement can tear the txn down
    // even when the JS caller catches the error.
    const cols = COLS.filter((c) => (row as Partial<ArticleRow>)[c] !== undefined);
    const useHintId = row.id !== undefined && cols.includes("id");
    if (useHintId) {
      const collision = await tx.get<{ id: number }>("SELECT id FROM articles WHERE id = ?", [
        row.id,
      ]);
      if (!collision) {
        const placeholders = cols.map(() => "?").join(", ");
